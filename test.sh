...
fixfinder () {
    ...
    info "Configuring list view settings for all folders..."
...
run_installers() {
    ...
    if in_array "Minimalist" "${nonbrew[@]}"; then
        info "Applying \"Minimalist\" preset..."
        minimalist
    fi

    if in_array "No Animations" "${nonbrew[@]}"; then
        info "Applying \"No Animations\" preset..."
        noanimation
    fi
    if in_array "Revamped Finder" "${nonbrew[@]}"; then
        info "Applying \"Revamped Finder\" preset..."
        fixfinder
    fi
}
