...
CATEGORY_Utilities=( "neovim" "fastfetch" "lf" "yt-dlp" "btop" "ffmpeg" "mono" "armadillo" "lazygit" "llvm" "python" "node" "openjdk" "lua" "qemu" "autojump")
CATEGORY_Casks=( "kitty" "helium" "brave-browser" "karabiner-elements" "ice" "middleclick" "stats" "linearmouse" "pearcleaner" "yellowdot" "iina" "command-x" "alt-tab" "prismlauncher" "bluestacks" "discord" "gimp" "obs" "qutebrowser")
...
run_installers() {
    ...
    if (( ${#formulae[@]} )); then
        ...
        for f in "${formulae[@]}"; do
            if [[ $f == "agkozak-zsh-prompt" ]]; then; continue; fi
            install_formula "$f"
        done
    else
...
# ================================ .zshrc ================================== #
    ...
    echo "" >> "$DOTS_DIR/.zshrc"
    echo "# ================= generated by config =================" >> "$DOTS_DIR/.zshrc"
    if in_array "neovim" "${formulae[@]}"; then
        echo 'export EDITOR="nvim"' >> "${DOTS_DIR}/.zshrc"
        echo 'export SUDO_EDITOR="$EDITOR"' >> "$DOTS_DIR/.zshrc"
    fi
    if in_array "agkozak-zsh-prompt" "${formulae[@]}"; then
        git clone https://github.com/agkozak/agkozak-zsh-prompt $DOTS_DIR/themes/agkozak-zsh-prompt
        echo "source $DOTS_DIR/themes/agkozak-zsh-prompt/agkozak-zsh-prompt.plugin.zsh" >> "$DOTS_DIR/.zshrc"
    fi
    if in_array "zsh-syntax-highlighting" "${formulae[@]}"; then
        echo 'source /opt/homebrew/share/zsh-syntax-highlighting/zsh-syntax-highlighting.zsh' >> "$DOTS_DIR/.zshrc"
    fi
    if in_array "zsh-autosuggestions" "${formulae[@]}"; then
        echo 'source $(brew --prefix)/share/zsh-autosuggestions/zsh-autosuggestions.zsh' >> "$DOTS_DIR/.zshrc"
    fi

    echo 'alias shrl="source ~/.zshrc"' >> "$DOTS_DIR/.zshrc"

    if in_array "autojump" "${formulae[@]}"; then
        echo '[ -f /opt/homebrew/etc/profile.d/autojump.sh ] && . /opt/homebrew/etc/profile.d/autojump.sh' >> "$DOTS_DIR/.zshrc"
    fi
    if in_array "fastfetch" "${formulae[@]}"; then
        echo 'alias ff="fastfetch"' >> "$DOTS_DIR/.zshrc"
    fi

    echo 'eval "$(/opt/homebrew/bin/brew shellenv)"' >> "$DOTS_DIR/.zshrc"

cat >> "$DOTS_DIR/.zshrc" << 'EOF'
cd ()
{
    if [ -n "$1" ]; then
    builtin cd "$@" && ls
...
EOF
    ...
    echo "# ================= generated by config =================" >> "$DOTS_DIR/.zshrc"
    ok ".zshrc successfully generated."
